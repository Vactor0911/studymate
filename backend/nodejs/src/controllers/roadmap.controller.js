...
class RoadMapController {
    static async createRoadMap(req, res) {
        try {
            ...
            const result = await roadMapService.createRoadMap(userId, subject, grade, learning_goal, study_duration);
            res.status(201).json({
    ...
    static async getRoadMap(req, res) {
        try {
            ...
            const roadMap = await roadMapService.getRoadMap(roadmapUuid, userId);
